...
const showWebNotification = message => async (dispatch, getState) => {
...
const onEntering = notification => async (dispatch, getState) => {
  const state = getState();
...
const onUser = notification => (dispatch, getState) => {
  const state = getState();
...
const onSubscription = notification => (dispatch, getState) => {
  const state = getState();
...
const onOrganization = notification => (dispatch, getState) => {
  const state = getState();
...
const notificationReceived = notification => (dispatch, getState) => {
  ...
  if (notification.object_type === 'subscription' || notification.object_name === 'subscription') {
    onSubscription(notification)(dispatch, getState);
  }
  ...
  if (notification.object_type === 'user') {
    onUser(notification)(dispatch, getState);
  }

  if (notification.event === 'entering') {
    onEntering(notification)(dispatch, getState);
  }
};
