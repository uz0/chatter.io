...
class EditTask extends Component {
  ...
  onTitleBlur = async () => {
    ...
    const { task } = await api.updateTask({ task_id: this.props.options.task_id, title: this.props.title.value });
  ...
  onDescriptionBlur = async () => {
    ...
    this.setState({ isLoading: true });
    const { task } = await api.updateTask({ task_id: this.props.options.task_id, description: this.props.description.value });
    this.setState({ isLoading: false });
    this.props.updateTask(task);
  };
    ...
    >
      ...
      <Form model="edit_task" className={style.form}>
        ...
        <div className={style.comments}>
          <button type="button" className={style.show_all}>Show all comments (33)</button>
        </div>
